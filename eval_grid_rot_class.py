...
# Y contains the same grid rotated either 90 or 180 degrees randomly
# Each row ends with token 10, and sequence ends with token 11 (EOS)
# For 5x5 grids, remaining tokens are padded with 12 (PAD)
# The task is to classify whether Y is a 90 or 180 degree rotation of X
def generate_data(num_samples=1000):
    ...
    while i < num_samples:
        ...
        # Randomly choose between 90 and 180 degree rotation
        rotation_type = torch.randint(0, 2, (1,)).item()  # 0 for 90 degrees, 1 for 180 degrees
        
        # Rotate grid according to chosen rotation
        rotated_grid90 = torch.rot90(grid, k=-1, dims=[0,1])
        rotated_grid180 = torch.rot90(grid, k=2, dims=[0,1])

        if torch.equal(rotated_grid90, rotated_grid180):
            # Under-determined grid... don't keep it.
            continue

        # Convert rotated grid to sequence with row markers
        pos = 0
        for row in range(N):
            if rotation_type == 0:
                Y[i, pos:pos+N] = rotated_grid90[row]  # Add rotated row values
            else:
                Y[i, pos:pos+N] = rotated_grid180[row]  # Add rotated row values
        ...
        labels[i, 1] = rotation_type  # 0 for 90 degrees, 1 for 180 degrees
        i += 1
            ...
            if verbose:
                ...
                for batch_idx in range(predictions.size(0)):
                    ...
                    if not batch_matches.all():
                        ...
                        print(f"Predicted rotation: {predictions[batch_idx][1].item()}")
                        print(f"Ground truth rotation: {Y_batch[batch_idx][1].item()}")
                        ...
                        ax2.set_title(f'Target Grid (Rotation: {y[1]})')
                        ax2.axis('off')
