...
# Y contains the same grid rotated 90 degrees clockwise
...
def generate_data(num_samples=1000):
    ...
    for i in range(num_samples):
        ...
        # Rotate grid 90 degrees clockwise for Y
        rotated_grid = torch.rot90(grid, k=-1, dims=[0,1])
        
        # Convert rotated grid to sequence with SOS token and row markers
        Y[i, 0] = SOS_token
        pos = 1  # Start after SOS token
        for row in range(N):
            Y[i, pos:pos+N] = rotated_grid[row]  # Add rotated row values
